...
public final class SkinUtil {
    ...
    public static String getTextures(Player player) {
        try {
            SkinsRestorer skinsRestorer = MagicPlayer.getInstance().getSkinsRestorer();
            if (skinsRestorer == null) {
                return DEF_TEXTURES;
            }
            SkinProperty skinProperty = skinsRestorer.getPlayerStorage()
            ...
            MagicPlayer.getInstance().getLoggerManager().warning(
                    "获取玩家" + player.getName() + "皮肤材质时出现错误: " + e.getMessage());
        }
        return DEF_TEXTURES;
    }

    public static String getSkin(String base64Texture) {
        try {
    ...
    public static String getAvatar(String base64Skin) {
        try {
    ...
    private static String drawAvatar(String base64Skin) throws IOException {
        if (base64Skin == null) {
            return DEFAULT_AVATAR;
        }
    ...
    public static ItemStack getHead(String base64Texture) {
